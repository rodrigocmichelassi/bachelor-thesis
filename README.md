...
`./src/scripts/data-generation.py`: This Python script is used to generate captions for the BRSET dataset. It has three main execution modes
...
- Class Captions: use the `.csv` file prepared in the first execution mode, with the CLIP ensemble technique, to generate 3 captions that describe if the retinal image is healthy or contains any disease. These captions are simpler, used for classification tasks.
     
To prepare the dataset for classification or image retrieval, run:
